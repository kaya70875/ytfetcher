...
def test_comment_fetcher_initialization():
    ...
    fetcher = CommentFetcher(max_comments=50, max_workers=5)
...
def test_fetch_single_success(mock_ydl_class, mock_comment_data):
    """Test the internal _fetch_single method logic."""
    ...
    results = fetcher._fetch_single("video123")
...
@patch.object(CommentFetcher, '_fetch_single')
def test_fetch_batch_logic(mock_fetch_single):
    ...
    fetcher = CommentFetcher(max_workers=2)
...
def test_fetch_single_empty_comments(mocker, mock_comment_data):
    ...
    fetcher = CommentFetcher()
    results = fetcher._fetch_single("no_comments_vid")
